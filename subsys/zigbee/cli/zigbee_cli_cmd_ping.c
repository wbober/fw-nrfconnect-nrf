...
static zb_void_t dispatch_user_callback(zb_bufid_t bufid)
{
    ...
    p_cmd_ping_status = ZB_BUF_GET_PARAM(bufid, zb_zcl_command_send_status_t);
...
static void ping_cli_evt_handler(ping_time_evt_t evt, zb_uint32_t delay_us, ping_request_t * p_request)
{
    switch (evt)
    {
        ...
        case PING_EVT_ERROR:
            print_error(p_request->p_cli, "Unable to send ping request", ZB_TRUE);
            break;
...
zb_void_t ping_request_send(ping_request_t * p_request)
{
    ...
    *(p_cmd_buf++) = m_ping_seq_num; /* Sequence Number Field */
    ...
    {
        zb_err_code = zb_zcl_finish_and_send_packet(bufid, p_cmd_buf, &(p_request->remote_addr),
    ...
    {
        zb_zcl_finish_and_send_packet_no_ack(bufid, p_cmd_buf, &(p_request->remote_addr),
                                             p_request->remote_addr_mode, cli_ep, cli_ep,
                                             ZB_AF_HA_PROFILE_ID, PING_CUSTOM_CLUSTER,
                                             dispatch_user_callback);
    }
...
static zb_void_t ping_reply_send(ping_reply_t * p_reply)
{
    ...
    *(p_cmd_buf++) = PING_ECHO_REPLY;
    ...
    if (p_reply->send_ack)
    {
        zb_err_code = zb_zcl_finish_and_send_packet(bufid, p_cmd_buf, (zb_addr_u *)(&(p_reply->remote_short_addr)),
    ...
    {
        zb_zcl_finish_and_send_packet_no_ack(bufid, p_cmd_buf, (zb_addr_u *)(&(p_reply->remote_short_addr)),
                                             ZB_APS_ADDR_MODE_16_ENDP_PRESENT, cli_ep, cli_ep,
                                             ZB_AF_HA_PROFILE_ID, PING_CUSTOM_CLUSTER,
                                             frame_acked_cb);
    }
...
static void ping_req_indicate(zb_bufid_t zcl_cmd_bufid)
{
    ...
    zb_zcl_parsed_hdr_t * p_cmd_info = ZB_BUF_GET_PARAM(zcl_cmd_bufid, zb_zcl_parsed_hdr_t);
...
void cmd_zb_ping(nrf_cli_t const * p_cli, size_t argc, char **argv)
{
    ...
    {
        print_error(p_cli, "Incorrect ping payload size", ZB_FALSE);
        zb_ping_release_request(p_row);
        return;
    }
