...
The light switch device operates through pressing buttons on the DK on the Zigbee network, and through the Nordic UART Service on the BLE network. Both networks are independent from each other.
...
Assignments
===================================
You can control the devices either with the buttons on the development kits or the NUS UART commands in the nRF Toolbox application.
...
LED 1:
    Indicates whether a BLE central is connected to the NUS service.
...
#. Turn on the light switch development kit.
   When LED 3 turns on on the light switch development kit, it has become an End Device.
...
You can toggle the LED on the light bulb board by pressing **Button 1** and **Button 2** on the light switch board.
